...
`gui/` 里有一个图形向导，双击就能用，四步走完：备份 → 输密码解密 → 勾选联系人 → 导出到桌面。全程点按钮，不敲一行命令。

1. 双击 `gui/安装环境.command`（第一次跑一次，自动装依赖并生成 App）
2. 双击生成的 `gui/微信导出.app`，跟着向导走

底层还是同一个引擎，数据同样只在本机、不联网。命令行版和图形版任选。
